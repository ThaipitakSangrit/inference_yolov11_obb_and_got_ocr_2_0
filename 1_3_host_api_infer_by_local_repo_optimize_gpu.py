...
# Setup logging
logging.basicConfig(filename="ocr_api_error_log.log", level=logging.ERROR, 
...
# Load model YOLOv11
model = YOLO(r"D:\work\python\ocr_utl3_test_infer_yolov11_obb_and_use_got_ocr_2_0\models\YOLOv11\best.pt")

# Load model GOT OCR
local_path = r"D:\work\python\ocr_utl3_test_infer_yolov11_obb_and_use_got_ocr_2_0\models\GOT_OCR2_0"
...
async def predict(request: ImageRequest):  
    try:
        ...
        w, h = image_rgb.size
        image_rgb = image_rgb.resize((w*2, h*2), Image.NEAREST)
        
        # ✅ แปลงเป็น NumPy array เพื่อใช้กับ OpenCV
        image_np = np.array(image_rgb)  
        
        # Inference with YOLOv11 model
        with torch.no_grad():
            results = model.predict(image_rgb, batch=1)

        result = []
        for res in results:
            if res.obb is not None:
                # shape: (N, 8) คือ array ของ 8 จุด (x1, y1, x2, y2, x3, y3, x4, y4)
                polygons = res.obb.xyxyxyxy.cpu().numpy()
                del res
                
                # แปลงเป็นกล่องสี่เหลี่ยมตรง (4 จุด) -> (N, 4, 2)
                boxes = polygons.reshape(-1, 4, 2) 
                
                # 🛡️ check object detection found?
                if len(boxes) == 0:
                    processing_time = time.time() - start_time
                    return JSONResponse(status_code=200, content={"result": "No objects detected.", "Processing Time (seconds)": processing_time})
                ...
                for i, pts in enumerate(boxes_sorted):
                    ...
                    # clear resource
                    del image_cropped, warped, M, pts_dst, pts_src, content_text
                    gc.collect()

        del image_rgb, image_np, results
        gc.collect()
